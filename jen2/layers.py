...
class Attention(nn.Module):
    def __init__(self, dim:int,*,dim_head:int =64,heads: int = 8,context_dim:int =None):
        ...
        self.null_kv = nn.Parameter(torch.radn(2, dim_head))
        self.to_q = nn.Linear(dim, inner_dim,bias=False)
        self.to_kv = nn.Linear(dim,dim_head * 2, bias=False)
        self.to_context = nn.sequential(nn.LayerNorm(context_dim), nn.Linear(context_dim, dim_head * 2)) if exists else None
    ...
    def forward(self,x: torch.tensor,context: torch.tensor = None,mask: torch.tensor = None,attn_bias: torch.tensor =None) -> torch.tensor:
        ...
        q = q * self.scale
        nk, nv = repeat_many(self.null_kv.unbind(dm=-2), 'd -> b l d', b=b)
        ...
        if exists(mask):
            ...
            sim = sim.masked_fill(~mask,mask_neg_value)
...
class Block(nn.Module):
    ...
    def forward(self,x:torch.tensor,scale_shift:tuple[torch.tensor,torch.tensor,torch.tensor]=None) -> torch.tensor:
        x = self.groupnorm(x)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x:torch.tensor,context:torch.tensor,mask:torch.tensor=None) -> torch.tensor:
        ...
        if exists(mask):
            mask = F.pad(mask, (1,0), value=True)
            mask = rearrange(mask, 'b j -> b 1 1 j')
            sim = sim.masked_fill(~max,mask_neg_value)
        attn = sim.softmax(dim=-1,dtype=torch.float32)
...
class CrossEmbedLayer(nn.Module):
    def __init__(self,dim_in: int,
        ...
        stride: int=2):
        super().__init__()
        assert all([*map(lambda t: (t % 2) == (stride % 2), kernek_sizes)])
        ...
        for kernel, dim_scale in zip(kernel_sizes,dim_scales):
            self.convs.append(nn.Conv2d(dim_in,dim_scale,kernel,stride=stride))

    def forward(self,x:torch.tensor) -> torch.tensor:
        fmaps = tuple(map(lambda conv:conv(x), self.convs))
        return torch.cat(fmaps,dim=1)

def Downsample(dim:int, dim_out:int = None) -> nn.Conv2d:
...
class ResnetBlock(nn.Module):
    def __init__(self,dim:int,dim_out:int,*,time_cond_dim: int=None,groups:int = 8):
        super().__init__()
        self.time_mlp = None
        ...
        if exists(cond_attn = None):
            self.cross_attn = EinopsToAndFrom(
                'b c h w',
                'b (h w) c',
                CrossAttention(dim=dim_out,
