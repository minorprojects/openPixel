...
from einops import einops import rearrange
from einops.layers.torch import Rearrange
from einops_exts.torch import EinopsToAndFrom
from .helpers import cast_tuple,prob_mask_like
from .layers import (
      Attention,
      CrossEmbedLayer,
      Downsample,
...
class Unet(nn.Module):
  ...
  def __init__(
      ...
      later_attns: Union[bool,ruple] = True,
      attn_heads:int = 8,
  ...
  self.to_time_hiddens = nn.Sequential(
      SinusodialPosEmb(dim),
      nn.Linear(dim, time_cond_dim),
      nn.SiLU()
  )
  self.to_time_cond = nn.Sequential(
      nn.Linear(time_cond_dim, cond_dim * NUM_TIME_TOKENS),
      Rearrange('b (r d) ->  r d', r=NUM_TIME_TOKENS)
  ...
  self.to_lowres_time_tokens = nn.Sequential(
       nn.Linear(time_cond_dim, cond_dim * NUM_TIME_TOKENS),
       Rerrange('b (r d) -> b r d', r=NUM_TIME_TOKENS)
  )
  self.norm_cond = nn.Layernorm(cond_dim)
  self.text_embed_dim = text_embed_dim
  self.text_to_cond = nn.Linear(self.text_embed_dim,cond_dim)
  max_text_len = 256
  self.max_text_en = max-text_len
  self.null_text_embed = nn.Parameter(torch.radn(1,max_text_len,cond_dim))
  self.null_text_hidden = nn.Parameter(torch.radn(1,rime_cond_dim))
  ...
  self.init_conv = CrossEmbedLayer(channels if not lowres_cond else channels * 2,
                                   ...
                                   kernel_size=(3,7,15),
                                   stride=1)
  dim = [dim , *map(lambda m:dim * m, dim_mults)]
